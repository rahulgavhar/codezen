...
const statBlocks = [
  { label: "Interview-ready problems", value: "400+" },
  { label: "Supported languages", value: "4" },
  { label: "Avg. session uptime", value: "99.9%" },
  { label: "Time to schedule", value: "<5 min" },
];

const shippedFeatureCards = [
  {
    title: "Resume to Skills to Jobs",
    description:
      "Upload a resume in profile, auto-extract skills, and generate personalized job recommendations from the same workflow.",
    icon: BriefcaseBusinessIcon,
  },
  {
    title: "Contest Replay + Live Leaderboard",
    description:
      "Replay contest timelines with real-time leaderboard updates so participants and organizers can review momentum and ranking shifts.",
    icon: TrophyIcon,
  },
  {
    title: "Problem Progress Tracking",
    description:
      "Attempted and solved states are persisted per user and available directly in problem cards, detail pages, and filters.",
    icon: BarChart3Icon,
  },
  {
    title: "Profile Activity Intelligence",
    description:
      "Private and public profiles show contribution heatmaps and rating journeys sourced from real submission and rating-history data.",
    icon: ActivityIcon,
  },
  {
    title: "GraphQL Layer (Live)",
    description:
      "A GraphQL endpoint powers signed-in profile fetching with a dedicated meProfile query and Clerk-authenticated context.",
    icon: DatabaseIcon,
  },
];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 pb-12 sm:pb-20 pt-8 sm:pt-12">
        ...
        <section className="mt-12 sm:mt-16 rounded-xl sm:rounded-2xl border border-white/10 bg-slate-900/60 p-4 sm:p-8 backdrop-blur">
          <div className="flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl sm:text-3xl font-semibold text-slate-50">
                Production features already live in this build
              </h2>
              <p className="mt-2 text-xs sm:text-base text-slate-300 max-w-3xl">
                Beyond the core interview workspace, Codezen now includes profile intelligence,
                contest replay systems, job recommendation flows, and GraphQL-backed profile access.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {shippedFeatureCards.map(({ title, description, icon: Icon }) => (
              <div
                key={title}
                className="rounded-xl border border-white/10 bg-slate-950/70 p-4 shadow-lg shadow-black/20"
              >
                <div className="flex items-start gap-3">
                  <span className="rounded-lg bg-white/5 p-2 text-emerald-200 ring-1 ring-emerald-400/30">
                    <Icon size={18} />
                  </span>
                  <div className="min-w-0">
                    <h3 className="text-sm sm:text-base font-semibold text-slate-100">{title}</h3>
                    <p className="mt-1 text-xs sm:text-sm text-slate-300">{description}</p>
                  </div>
                </div>
              </div>
            ))}
